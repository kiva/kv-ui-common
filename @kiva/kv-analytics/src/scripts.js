export default () => {
	...
	const insertSnowplow = () => {
		...
		(function(p,l,o,w,i,n,g){if(!p[i]){p.GlobalSnowplowNamespace=p.GlobalSnowplowNamespace||[];
			...
			window.snowplow('newTracker', 'cf', process.env.snowplowUri, { // Initialize a tracker
				appId: 'kiva' ,
	...
	const insertGTM = () => {
		...
		})(window,document,'script','dataLayer',process.env.googleTagmanagerId);
		/* eslint-enable */
	};
	...
	// Always load
	// PerimeterX snippet
	if (process.env.enablePerimeterx) {
		/* eslint-disable */
		(function(){
			window._pxAppId = process.env.perimeterxAppId;
			var p = document.getElementsByTagName('script')[0],
				s = document.createElement('script');
			s.async = 1;
			s.src = `${window.location.origin}/${process.env.perimeterxAppId.substring(2)}/init.js`;
	...
	if (process.env.oneTrust && process.env.oneTrust.enable) {
		/* eslint-disable */
		(function () {
			...
			s.setAttribute('data-domain-script', `${process.env.oneTrust.key}${process.env.oneTrust.domainSuffix}`)
			s.src = `https://cdn.cookielaw.org/consent/${process.env.oneTrust.key}${process.env.oneTrust.domainSuffix}/otSDKStub.js`;
			p.parentNode.insertBefore(s, p);
		}());
		...
		window.OptanonWrapper = () => {
			...
			* */
			if (process.env.enableAnalytics) {
				if (process.env.enableGA && !optout) {
					OneTrust.InsertHtml('', 'head', insertGoogleAnalytics, null, 'C0002');
				}
				if (process.env.enableSnowplow) {
					OneTrust.InsertHtml('', 'head', insertSnowplow, null, 'C0002');
				}
				if (process.env.algoliaConfig.enableAA && !optout) {
					OneTrust.InsertHtml('', 'head', insertAlgoliaAnalytics, null, 'C0002');
				}

				if (process.env.enableFullStory && !optout) {
			...
			* */
			...
			if (process.env.enableAnalytics) {
				if (process.env.enableGTM && !optout) {
					OneTrust.InsertHtml('', 'head', insertGTM, null, 'C0004');
				}
				if (process.env.enableFB && !optout) {
